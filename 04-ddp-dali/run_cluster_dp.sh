...
function cleanup {
    ...
    echo "[System] change permission in docker container created files"
    docker exec ${DOCKER_NAME} bash -c "chmod -R 777 /HOME"
    ## docker container stop
    docker stop ${DOCKER_NAME}
    echo "[System] docker container stopped"
    ## docker container remove
    docker rm -f ${DOCKER_NAME}
    echo "[System] docker container removed"
    exit
}
...
docker exec ${DOCKER_NAME} bash -c "cd /HOME && bash scripts/run_dp.sh"
