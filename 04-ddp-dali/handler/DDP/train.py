...
def resume_checkpoint(model, optimizer, root, prefix='train'):  
    dist.barrier() # force sync for safety 
...
def sync_checkpoint(root, model):
    sync_path = os.path.join(root, "sync.tmp") 
    
    if dist.get_rank() == 0:        
        state = {
            "model": model.state_dict()
        }
        torch.save(state, sync_path)

    dist.barrier()
    
    map_location = {'cuda:%d' % 0: 'cuda:%d' % torch.cuda.current_device()}
    state = torch.load(sync_path, map_location=map_location)
...
def train(train_loader, model, criterion, optimizer, epoch):    
    ...
    with nvtx.annotate(f"Epoch {epoch}"):
        for i, (input, target) in enumerate(train_loader):  
            with nvtx.annotate("Train batch : {" + str(i) + "}"):
            ...
            if dist.get_rank() == 0 and ((i+1) % _print_freq) == 0:
                print('Epoch: [{0}][{1}/{2}]\t'
